...
export class AuthController {
    ...
    async login(request: Request, response: Response) {
        ...
        if (!email || !password) {
            return RouteResponse.error(response, 'email ou senha do usuário ausentes');
        }

        const existingUser: User | undefined = await userRepository.findUserByEmail(email);

        if (!existingUser) {
            return RouteResponse.notFound(response, 'Nome de usuário ou senha incorretos');
        }

        const validPassword: boolean = await bcrypt.compare(password, existingUser.password);
        if (!validPassword) {
            return RouteResponse.error(response, 'Nome de usuário ou senha incorretos');
        }

        const token: string = jwt.sign({ email: email }, process.env.JWT_SECRET as string, {
            expiresIn: '24h'
        });
        ...
        tokenRepository.save({ token, expiresAt, user: existingUser });
